        ...
        <Container>
          ...
          <div className="overflow-x-auto border border-navy/15 bg-cream">
            <table className="w-full min-w-[1050px] text-left text-sm">
              <thead className="border-b border-navy/15 bg-navy text-cream"><tr><th className="px-4 py-4">Date</th><th className="px-4 py-4">Type</th><th className="px-4 py-4">Name</th><th className="px-4 py-4">Email</th><th className="px-4 py-4">Phone</th><th className="px-4 py-4">Topic</th><th className="px-4 py-4">Message</th></tr></thead>
              <tbody>
                {rows.length ? rows.map((row) => <tr key={row.id} className="border-b border-navy/10 align-top last:border-0"><td className="whitespace-nowrap px-4 py-4 text-charcoal/70">{row.createdAt.toLocaleString()}</td><td className="px-4 py-4 capitalize text-charcoal/70">{row.inquiryType}</td><td className="px-4 py-4 font-semibold text-navy">{row.name || 'Not provided'}</td><td className="px-4 py-4 text-charcoal/70">{row.email || '—'}</td><td className="px-4 py-4 text-charcoal/70">{row.phone || '—'}</td><td className="px-4 py-4 text-charcoal/70">{row.subject || row.need || row.role || '—'}</td><td className="max-w-[360px] whitespace-pre-wrap px-4 py-4 text-charcoal/80">{row.message}</td></tr>) : <tr><td colSpan={7} className="px-4 py-12 text-center text-charcoal/60">No inquiries yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </Container>
